...
export async function generateMetadata(
    ...
    const blogPost = await client.fetch<PostType>(queries.blogBySlug, { slug: params.slug });
...
export default async function BlogPostPage(props: Props) {
    const params  = await props.params
    const blogPost = await client.fetch<PostType>(queries.blogBySlug, { slug: params.slug })
    console.log(JSON.stringify(blogPost.body, null, 2))
    if (!blogPost) {
        notFound()
    }
                ...
                <div className="relative h-[60vh] min-h-[400px] bg-gray-600">
                    ...
                    <div className="relative container mx-auto px-4 h-full flex items-end pb-16">
                        <div className="max-w-3xl">
                            <Link href="/blog" className="inline-flex items-center text-white mb-6 hover:text-purple-300 transition-colors">
                            ...
                            <div className="flex items-center space-x-4 text-white">
                                ...
                                <div>
                                    <p className="font-medium">{blogPost.author?.name?.toString() || 'Guest Post'}</p>
                                    <p className="text-sm opacity-75">
                                        {new Date(blogPost.publishedAt?.toString() || '').toLocaleDateString('en-US', {
                                            year: 'numeric',
                                            month: 'long',
                                            day: 'numeric',
                                        })}
                            ...
                            <div className="hidden lg:block w-64">
                                <div>
                                    ...
                                    <div className="bg-white p-6 rounded-lg shadow-sm mb-6">
                                        <h3 className="text-sm font-medium text-gray-500 mb-4">WRITTEN BY</h3>
                                        <div className="flex items-center space-x-4">
                                            {blogPost.author?.image? (
                                            ...
                                            <div>
                                                <p className="font-medium">{blogPost.author?.name?.toString() || 'Guest Post'}</p>
                                                {blogPost.author?.bio && (
                                                    <p className="text-sm text-gray-600 mt-1">{blogPost.author.bio}</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {blogPost.categories && blogPost.categories.length > 0 && (
                                        <div className="bg-white p-6 rounded-lg shadow-sm">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {blogPost.categories.map((category: { title: string }) => (
                                                    <span
                                                        key={category.title}
